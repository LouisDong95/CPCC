...
class LoggerX(object):
    ...
    def __init__(self, save_root, enable_wandb=False, resume_wandb=False, **kwargs):
        ...
        if enable_wandb and self.local_rank == 0:
            import wandb
            wandb.init(dir=save_root, settings=wandb.Settings(_disable_stats=True, _disable_meta=True), **kwargs, resume=resume_wandb)
    ...
    def checkpoints(self, epoch):
        ...
        for i in range(len(self.modules)):
            ...
            torch.save(module.state_dict(), osp.join(self.models_save_dir, '{}'.format(module_name)))

    def load_checkpoints(self, epoch):
        for i in range(len(self.modules)):
            module_name = self.module_names[i]
            module = self.modules[i]
            module.load_state_dict(load_network(osp.join(self.models_save_dir, '{}'.format(module_name))))
