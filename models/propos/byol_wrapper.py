...
class BYOLWrapper(MoCoWrapper):
    ...
    def compute_centers(self, x, psedo_labels, weights_all):
        # n_samples = x.size(0)
        # if len(psedo_labels.size()) > 1:
        #     index = psedo_labels.T
        # else:
        #     index = torch.zeros(self.num_cluster, n_samples).to(x)  # L, N
        #     index[psedo_labels, torch.arange(n_samples)] = 1
        # index = index * weights_all.T
        # weight_index = F.normalize(index, p=1, dim=1)  # l1 normalization
        # centers = torch.mm(weight_index, x)
        centers = torch.mm(weights_all.T, x)
    ...
    def forward_k(self, im_k, psedo_labels, weights):
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            if self.queue_size > 0:
                ...
                k_centers = self.compute_centers(all_k, psedo_labels, weights)
        return k, k_centers, all_k


    def forward_loss(self, im_q, im_k, psedo_labels: torch.Tensor, weights, high_confidence_indices):
        # compute query features
        q = self.encoder_q(im_q)  # queries: NxC
        q = self.projector_q(q)

        batch_psedo_labels = psedo_labels
        batch_psedo_weights = weights
        batch_all_psedo_labels = self.concat_all_gather(batch_psedo_labels)
        batch_all_psedo_weights = self.concat_all_gather(batch_psedo_weights)
        k, k_centers, all_k = self.forward_k(im_k, batch_all_psedo_labels, batch_all_psedo_weights)

        noise_q = q + torch.randn_like(q) * self.latent_std

        contrastive_loss = - 2 * F.cosine_similarity(self.predictor(noise_q), k).mean() - 2 * F.cosine_similarity(self.predictor(q), k).mean()

        # if len(high_confidence_indices) > 0:
        #     high_q = q[high_confidence_indices]
        #     high_k = k[high_confidence_indices]
        #     contrastive_loss_high = - 2 * F.cosine_similarity(self.predictor(high_q), high_k).mean()
        # else:
        #     contrastive_loss_high = torch.tensor([0.0]).cuda()
        ...
        if self.queue_size > 0:
            ...
            q_centers = self.compute_centers(all_q, batch_all_psedo_labels, weights)
    ...
    def forward(self, im_q, im_k, indices, momentum_update=True, v2=False):
        ...
        high_confidence_indices = torch.where(weights > 0.6)[0]
        if self.symmetric:
            contrastive_loss1, cluster_loss_batch1, feature_loss_batch1, q1, k1 = self.forward_loss(im_q, im_k, psedo_labels, weights, high_confidence_indices)
            contrastive_loss2, cluster_loss_batch2, feature_loss_batch2, q2, k2 = self.forward_loss(im_k, im_q, psedo_labels, weights, high_confidence_indices)
            contrastive_loss = 0.5 * (contrastive_loss1 + contrastive_loss2)
            cluster_loss_batch = 0.5 * (cluster_loss_batch1 + cluster_loss_batch2)
            feature_loss_batch = 0.5 * (feature_loss_batch1 + feature_loss_batch2)
            q = torch.cat([q1, q2], dim=0)
            k = torch.cat([k1, k2], dim=0)
        else:  # asymmetric loss
            contrastive_loss, cluster_loss_batch, feature_loss_batch, q, k = self.forward_loss(im_q, im_k, psedo_labels, weights, high_confidence_indices)
        ...
        return contrastive_loss, cluster_loss_batch, feature_loss_batch, q
    ...
    def forward_v2(self, im_q_, im_k_, indices, momentum_update=True):
        ...
        batch_psedo_labels = psedo_labels
        batch_all_psedo_labels = self.concat_all_gather(batch_psedo_labels)
        k, _, all_k = self.forward_k(im_k, batch_all_psedo_labels.repeat(2), weights.repeat(2, 1))
        k1, k2 = all_k.chunk(2, dim=0)
        k_centers_1 = self.compute_centers(k1, batch_all_psedo_labels, weights)
        k_centers_2 = self.compute_centers(k2, batch_all_psedo_labels, weights)

        noise_q = q + torch.randn_like(q) * self.latent_std

        # contrastive_loss = (2 - 2 * F.cosine_similarity(self.predictor(noise_q), k)).mean()
        contrastive_loss = (1 - F.cosine_similarity(self.predictor(noise_q), k)).mean() + (1 - F.cosine_similarity(self.predictor(q), k)).mean()

        all_q = F.normalize(torch.cat(GatherLayer.apply(q), dim=0), dim=1)
        q1, q2 = all_q.chunk(2, dim=0)
        q_centers_1 = self.compute_centers(q1, batch_all_psedo_labels, weights)
        q_centers_2 = self.compute_centers(q2, batch_all_psedo_labels, weights)
    ...
    def mine_nearest_neighbors(self, topk, features, calculate_accuracy=True):
        import faiss
        ...
        index.add(features)
        distances_nea, indices_nea = index.search(features, topk + 1)

        # evaluate
        if calculate_accuracy:
            targets = self.targets.cpu().numpy()
            neighbor_targets = np.take(targets, indices_nea[:, 1:], axis=0)  # Exclude sample itself for eval
            anchor_targets = np.repeat(targets.reshape(-1, 1), topk, axis=1)
            accuracy_nea = np.mean(neighbor_targets == anchor_targets)

            return indices_nea, accuracy_nea

        else:
            return indices_nea

    def local_neighbor_loss(self, topk, q, k, index):
        rows, cols = index.shape
        # q = q.repeat(1, topk).view(-1, dim)
        random_index = np.random.choice(cols, rows)
        index = index[np.arange(rows), random_index]
        k = k[index.reshape(-1,1)].squeeze()
        loss = (1 - F.cosine_similarity(self.predictor(q), k)).mean()
        return loss
