...
from utils import TwoCropTransform, extract_features, tsne_plot
...
class TrainTask(object):
    ...
    def build_default_options():
        ...
        parser.add_argument('--entity', help='wandb project_name', type=str, default='louis')
        parser.add_argument('--run_name', type=str, help='each run name')

        parser.add_argument('--num_workers', type=int, default=16, help='num of workers to use')
        parser.add_argument('--resume_epoch', type=int, default=499, help='number of training epochs')
        parser.add_argument('--resume_name', type=str, default='CPCC2_temp0.9')
        ...
        parser.add_argument('--data_folder', type=str, default='../../Datasets/Images',
    ...
    def build_dataloader(self,
        ...
        dataset, has_subfolder, class_names = self.create_dataset(data_root, dataset_name,
    ...
    def set_loader(self):
        ...
        self.weights = torch.zeros((self.num_samples, self.num_cluster)).cuda()
    ...
    def train_global(self, n_iter, train_loader):
        pass
    ...
    def clustering(self, features, n_clusters):
        ...
        # cluster = cluster_centers.unsqueeze(0)
        # dist_sq = torch.sum((features - cluster) ** 2, dim=2)
        # weights = torch.exp(-dist_sq)
        # if not opt.center_weights or self.cur_epoch <= 500:
        #     weights = torch.ones_like(weights)

        weights = self.cal_weights(features, cluster_centers)
        return psedo_labels, cluster_centers, weights
    ...
    def psedo_labeling(self, n_iter):
        ...
        if self.cur_epoch == 200 or self.cur_epoch == 1000:
            tsne_plot(mem_features.cpu().numpy(), self.gt_labels, osp.join('./ckpt', opt.run_name, 'save_images/tsne{}'.format(self.cur_epoch)))

        psedo_labels, cluster_centers, weights = self.clustering(mem_features, self.num_cluster)
        dist.barrier()
        ...
        confusion_matrix_file = osp.join('./ckpt', opt.run_name, 'save_images/confusion_matrix') if self.cur_epoch == 1000 else None

        results = torch_clustering.evaluate_clustering(mem_labels.cpu().numpy(),
                                                       psedo_labels.cpu().numpy(),
                                                       eval_metric=opt.eval_metric,
                                                       phase='ema_train', class_names=self.class_names, confusion_matrix_file=confusion_matrix_file)
    ...
    def cal_weights(self, features, centers):
        q = 1/(torch.square(torch.cdist(features, centers)) + 1)
        q = q/torch.sum(q, 1, keepdim=True)
        p = torch.square(q)/torch.sum(q, 0, keepdim=True)
